@if (Auth::user()?->role == 'admin')
    <header class="flex justify-between  px-40 py-8 border-b-2 pb-8 fixed w-100 top-0 left-0 right-0 bg-white ">
        ...
        <div class="flex gap-2 items-center">
            <img class="object-contain rounded-full w-10 h-10"
                src="/Images/{{ Auth::user()?->customer->profile_image ?? 'default.jpg' }}" alt='profile'>
...
@else
    <header class="flex justify-between  px-40 py-8 border-b-2 pb-8 fixed w-100 top-0 left-0 right-0 bg-white ">
        ...
        <div class="flex gap-2 items-end">
            ...
            <a class="w-7 aspect-square" href="{{ route('cart') }}"><img src="/assets/icon-cart.svg"></a>
                @if (Auth::user())
                    <img class="object-contain rounded-full w-10 h-10"
                        src="/Images/{{ Auth::user()?->customer->profile_image ?? 'default.jpg' }}" alt='profile'>
                @endif
